...
var joiner = require('../dist/joiner.cjs.js')
...
//
var joinedResult = '{"data":[{"id":"1","name":"UT","avg_temp":72},{"id":"2","name":"WY","avg_temp":null},{"id":"3","name":"CO","avg_temp":34},{"id":"4","name":"NM","avg_temp":45}],"report":{"diff":{"a":["CO","NM","UT","WY"],"b":["CO","NM","UT"],"a_and_b":["CO","NM","UT"],"a_not_in_b":["WY"],"b_not_in_a":[]},"prose":{"summary":"3 rows matched in A and B. 1 row in A not in B. All 3 rows in B matched in A.","full":"Matches in A and B: CO, NM, UT. A not in B: WY."},"matchStatus":"all-in-b"}}'
var joinedResultNested = '{"data":[{"id":"1","name":"Utah","values":{"weather":{"avg_temp":72}}},{"id":"2","name":"Wyoming","values":{"weather":{"avg_temp":null}}},{"id":"3","name":"Colorado","values":{"weather":{"avg_temp":34}}},{"id":"4","name":"New Mexico","values":{"weather":{"avg_temp":45}}}],"report":{"diff":{"a":["Colorado","New Mexico","Utah","Wyoming"],"b":["Colorado","New Mexico","Utah"],"a_and_b":["Colorado","New Mexico","Utah"],"a_not_in_b":["Wyoming"],"b_not_in_a":[]},"prose":{"summary":"3 rows matched in A and B. 1 row in A not in B. All 3 rows in B matched in A.","full":"Matches in A and B: Colorado, New Mexico, Utah. A not in B: Wyoming."},"matchStatus":"all-in-b"}}'
...
describe('js api', function () {
  describe('json', function () {
    it('should match expected json', function () {
      ...
      expect(joinedData).to.deep.equal(JSON.parse(joinedResult))
    })
    ...
    it('should match expected json, nesting under key', function () {
      ...
      var joinedData = joiner(config)
      expect(joinedData).to.deep.equal(JSON.parse(joinedResultNested))
    })
    ...
    it('should match expected json, with nested join keys', function () {
      ...
      var joinedData = joiner(config)
      expect(joinedData).to.deep.equal(JSON.parse(joinedResultNestedKeys))
    })
    ...
    it('should match expected json, with no matches', function () {
      ...
      var joinedData = joiner(config)
      expect(joinedData).to.deep.equal(JSON.parse(noMatchDataResult))
    })
    ...
    it('should match expected json, with perfect matches', function () {
      ...
      var joinedData = joiner(config)
      expect(joinedData).to.deep.equal(JSON.parse(perfectDataResult))
    })
  })
